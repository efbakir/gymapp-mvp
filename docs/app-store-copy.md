...
Raw workout details remain on-device via SwiftData. Unit sends a small allowlist of anonymous product-interaction and purchase-state events through TelemetryDeck. Events contain only controlled categories and buckets; they never contain exercise names, program text, weights, reps, notes, bodyweight, email, exact workout times, raw workout identifiers, or StoreKit receipts. TelemetryDeck processes a privacy-preserving app-scoped identifier so anonymous activity can be counted. Analytics are not linked to identity, are not used for tracking, and can be disabled immediately in Settings → Anonymous analytics. Unit remains fully functional offline and when analytics configuration is unavailable.
