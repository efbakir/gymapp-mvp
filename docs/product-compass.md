...
| 2026-05-10 | **History is list-only; scheduling stays lightweight** | History is a chronological session list. Optional weekday assignment or flexible rotation helps Today choose a template, but templates remain the program unit. | Calendar tab and calendar-as-program assumptions |
...
**Execution constraints (still active):** Swift 6, SwiftUI, SwiftData, local-first. Atomic design system. Light-only. No CloudKit sync at v1. No exercise discovery feed.
